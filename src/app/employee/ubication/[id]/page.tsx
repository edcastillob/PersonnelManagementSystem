...
const EditUbicationPage = ({ params }: { params: Params }) => {
  ...
  async function fetchPosition() {
    try {
      const resp = await axios.get(`/api/employee/ubication/${params.id}`);
  ...
  const onSubmit = handleSubmit(async (data) => {
    const updatePosition = await axios.put(
      `/api/employee/ubication/${params.id}`,
      data
    );
    reset();
    if (updatePosition.statusText) {
      toast.info("Ubication Edited");
      setTimeout(() => {
        router.refresh();
        router.push("/employee/ubication/all");
      }, 1500);
    }
  });
    ...
    <div className="h-[calc(100vh-7rem)] flex justify-center items-center">
      {loading ? (       
        ...
        <form onSubmit={onSubmit} className="w-1/4">
          <h1 className="text-slate-200 font-bold text-4xl mb-4">Ubication</h1>
          ...
          <div className="flex justify-between">
            ...
            <button
              ...
              type="button"
              onClick={() => router.push("/employee/ubication/all")}
            >
              Cancel
            </button>
          </div>
...
export default EditUbicationPage;
